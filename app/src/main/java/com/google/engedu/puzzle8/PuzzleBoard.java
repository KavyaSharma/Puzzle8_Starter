...
public class PuzzleBoard {
        ...
        for (int y = 0; y < NUM_TILES; y++) {
            for (int x = 0; x < NUM_TILES; x++) {
                ...
                if (num != NUM_TILES * NUM_TILES - 1) {
                    Bitmap titleBitmap = Bitmap.createBitmap(scaledBitmap,
                            x * scaledBitmap.getWidth() / NUM_TILES,
                            y * scaledBitmap.getWidth() / NUM_TILES,
                            parentWidth / NUM_TILES,
                            parentWidth / NUM_TILES);
                    PuzzleTile tile = new PuzzleTile(titleBitmap, num);
                    tiles.add(tile);
                } else
                    tiles.add(null);
            }
    ...
    public ArrayList<PuzzleBoard> neighbours() {
        ...
        int x = nulltile % NUM_TILES, y = nulltile / NUM_TILES;
        for (int i = 0; i < NEIGHBOUR_COORDS.length; i++) {
            ...
            if (tempX < NUM_TILES && tempX >= 0 && tempY < NUM_TILES && tempY >= 0) {
                ...
                otherBoard.swapTiles(nulltile, XYtoIndex(tempX, tempY));
                neighbours.add(otherBoard);
            }
        }return neighbours;
    }
    public int priority() {
